...
def plot_hydrogen_basis() -> None:
    ...
    fig, ax0, _ = plot_explicit_basis_states_x(hamiltonian["basis"][2])

    potential = get_interpolated_potential(shape)
    minimum = select_minimum_potential_3d(potential)
    _, ax1, _ = plot_potential_1d_x(minimum, 0, (), ax=ax0.twinx())
    ...
    fig, ax, _ = plot_explicit_basis_states_x(hamiltonian["basis"][2], measure="imag")
    fig.show()
    input()
