...
def calculate_rates_hydrogen(
    ...
    *,
    ...
    for i, t in enumerate(temperatures):
        ...
        if plot:
            fig, ax, _ = plot_isf_fit_against_time(fit, times)
            plot_isf_against_time(isf, times, ax=ax)
            fig.show()
            input(fit)
...
def plot_tunnelling_rate_hydrogen() -> None:
    ...
    fast_rates, slow_rates = calculate_rates_hydrogen(
        temperatures, n_bands=2, plot=True
    )
    (line,) = ax.plot(temperatures, fast_rates)
...
def plot_relevant_rates_hydrogen() -> None:
    a_matrix = get_tunnelling_a_matrix_hydrogen((5, 5), 6, 150)
    m_matrix = get_tunnelling_m_matrix(a_matrix, n_bands=2)
    initial = get_initial_pure_density_matrix_for_basis(m_matrix["basis"], (0, 0, 0))
    temperatures = np.array([125, 150, 175, 200, 225])
    rates = [
        get_rate_decomposition(
            get_tunnelling_m_matrix(
                get_tunnelling_a_matrix_hydrogen((5, 5), 6, t), n_bands=2
            ),
            initial,
        )
        for t in temperatures
