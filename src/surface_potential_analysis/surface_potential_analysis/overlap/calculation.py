...
from surface_potential_analysis.eigenstate.eigenstate import (
    convert_eigenstate_01_axis_to_position_basis,
)
...
def calculate_overlap(
    wavepacket1: Wavepacket[
        _NS0Inv,
        _NS1Inv,
        BasisConfig[MomentumBasis[_L0Inv], MomentumBasis[_L1Inv], _BX0Inv],
    ],
    ...
    Overlap[BasisConfig[PositionBasis[int], PositionBasis[int], _BX0Inv]]
    """
    eigenstate1 = convert_eigenstate_01_axis_to_position_basis(
        unfurl_wavepacket(wavepacket1)
    )
    eigenstate2 = convert_eigenstate_01_axis_to_position_basis(
